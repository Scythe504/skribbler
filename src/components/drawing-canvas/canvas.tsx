...
export const PixelArtCanvas = () => {
  ...
  const currentDrawer = useMemo(() => gameState.current_drawer, [gameState.current_drawer]);

  // Drawing status message
  const drawingStatusMessage = useMemo(() => {
    if (!isDrawingPhase) return `Phase: ${gameState.phase}`;
    if (!currentDrawer) return "No drawer selected";
    if (currentDrawer.username === playerName) return "Your turn to draw!";
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full overflow-visible">
      ...
      <div className="text-center">
        ...
        {Number(gameState.time_remaining) > 0 && (
          <div className="text-sm text-gray-600">
            Time remaining: {Math.ceil(Number(gameState.time_remaining) / 1000)}s
          </div>
        )}
      </div>
      ...
      <div className="text-sm text-gray-600 mb-2">
        ...
        {!canDraw && isDrawingPhase && (
          <span className="ml-2 text-red-500 font-medium">(Spectator mode)</span>
        )}
      </div>
      ...
      {/* Drawing Indicator */}
      {isDrawing && canDraw && (
        <div className="text-xs text-green-600 font-medium">Drawing...</div>
      )}
    </div>
  );
};
